...
\subsection{Unbalanced Panel (Intended Extension)}
...
Aggregates become NA only if every DMU is missing for that slice; a single incomplete DMU does not invalidate the slice's statistics.
...
\begin{remark}
The unbalanced extension described here is an \emph{intended} feature, not yet implemented in the current \texttt{analyze\_panel} function. The shipped code enforces complete data and raises \texttt{ValueError} if any entry is missing. This section documents the mathematical framework for a future release.
\end{remark}
